...
def _get_routes(api):
    ...
    def list_worksets(uiapp):
        ...
        for ws in FilteredWorksetCollector(doc).OfKind(WorksetKind.UserWorkset):
            results.append({'name': _safe_name(ws), 'workset_id': _idv(ws.Id), 'is_open': ws.IsOpen, 'owner': ws.Owner or None})
    ...
    def set_element_workset(uiapp, request):
        ...
        target_ws = next((ws for ws in FilteredWorksetCollector(doc).OfKind(WorksetKind.UserWorkset) if _safe_name(ws) == ws_name), None)
        if not target_ws:
    ...
    def set_active_workset(uiapp, request):
        ...
        if not target_ws:
            return Response(status_code=404, data={'error': "Workset '{}' not found".format(ws_name)})
        table = doc.GetWorksetTable()
        old_active = _safe_name(table.GetWorkset(table.GetActiveWorksetId()))
